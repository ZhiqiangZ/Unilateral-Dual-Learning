...
from model import RNN_ENCODER, CNN_ENCODER, CAPTION_CNN, CAPTION_RNN  #add by zzq
...
class condGANTrainer(object):
    ...
    def build_models(self):
        ...
        # Caption models - cnn_encoder and rnn_decoder   (add by zzq)
        ...
        if cfg.CUDA:
            ...
            for i in range(len(netsD)):
                netsD[i].cuda()
        return [text_encoder, image_encoder, caption_cnn, caption_rnn, netG, netsD, epoch]  #modify by zzq
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            while step < self.num_batches:
                ...
                for i in range(len(netsD)):
                    ...
                    D_logs += log  #can consider to remove it (by zzq)
                ...
                if gen_iterations % 1000 == 0:
                    ...
                    self.save_img_results(netG, fixed_noise, sent_emb, words_embs, mask, image_encoder,
                                          captions, cap_lens, epoch, imgs[-1], name='current')
                    load_params(netG, backup_para)
                    #
